...
This standalone example demonstrates how to use a Python function to dynamically
determine input files based on sample metadata (normal/tumour pairing from `sample_ids.csv`).
```
...
Snakemake provides two useful output modifiers for managing disk space and protecting critical files:

- **`temp()`** — marks an output file as temporary. Snakemake will delete it once all downstream rules that depend on it have completed. Useful for large intermediate files (e.g. unsorted BAMs).
- **`protected()`** — marks an output file as protected. Snakemake will make it read-only after creation to prevent accidental modification or deletion. Useful for files that are expensive to regenerate.
...
rule mark_dups:
    ...
    output:
        bam = temp("data/dupmarked_bam/{sample}_dupmarked.bam"),
        metrics = protected("data/dupmarked_bam/{sample}_dupmetrics.txt")
    conda: "envs/gatk.yaml"
    shell: "gatk MarkDuplicates -I {input} -O {output.bam} -M {output.metrics}"
```
In this example, the duplicate-marked BAM is deleted after downstream rules finish (since only the metrics are needed for plotting), while the metrics file is protected from accidental deletion.

### Workflow reports with `--report`

Snakemake can generate an HTML report summarising your workflow run:
```
$ snakemake -j 4 --use-conda --report report.html
```
The report includes:
- Runtime statistics for each rule
- A visualisation of the workflow DAG
- File provenance (which rule created which file)
- Configuration used

This is useful for documenting your analysis and sharing results with collaborators.

### Checkpoint rules (conceptual)

Standard Snakemake rules require all input/output file names to be known before execution begins. **Checkpoint rules** relax this constraint — they allow the DAG to be re-evaluated after a rule completes.

This is useful when:
- The number of output files depends on the input data (e.g. splitting a file into an unknown number of chunks)
- You need to inspect intermediate results before deciding what to run next

A checkpoint is defined using `checkpoint` instead of `rule`:
```python
checkpoint split_samples:
    input: "data/all_samples.txt"
    output: directory("data/split/")
    shell: "split -l 100 {input} {output}/chunk_"
```

Downstream rules then use an **input function** to discover the checkpoint outputs at runtime. Checkpoint rules are an advanced feature — see the [Snakemake documentation on checkpoints](https://snakemake.readthedocs.io/en/stable/snakefiles/rules.html#data-dependent-conditional-execution) for full details.
